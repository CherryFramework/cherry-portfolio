  ...
  */
  ...
  $.fn.justifiedGallery = function (arg) {
    ...
    function imgFromEntry($entry) {
      var $img = $entry.find('img.justified-image');
      return $img;
    }

    function displayEntry($entry, x, y, imgWidth, imgHeight, rowHeight, context) {
      var $image = imgFromEntry($entry);
      $image.css('width', Math.round( imgWidth ) );
      $image.css('height', Math.round( imgHeight ) );

      //$image.css('margin-left', - imgWidth / 2);
      //$image.css('margin-top', - imgHeight / 2);

      $entry.width( Math.round( imgWidth ) );
      $entry.height( Math.round( rowHeight ) ) ;

      $entry.css({"position": "absolute", left: Math.round(x), top: Math.round(y) });
